...
Thread::Sleep (bool finishing)
{
    ...
    if (finishing == false){
        DEBUG('z',"[D] Tick ["<<kernel->stats->totalTicks<<"]: Thread ["<<kernel->currentThread->getID()<<"] update approximate burst time, from: ["<<kernel->currentThread->getPredict()
        <<"], add ["<<kernel->currentThread->getExecTime()<<"], to ["<<kernel->currentThread->getPredict() / 2 + kernel->currentThread->getExecTime() / 2<<"]\n");
    }
