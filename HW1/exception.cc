...
ExceptionHandler(ExceptionType which)
{
	...
	switch(type) {
		...
		{
		char *filename = &(kernel->machine->mainMemory[val]);
		int fd = Syswrite(filename,(int)kernel->machine->ReadRegister(5),(int)kernel->machine->ReadRegister(6));
		kernel->machine->WriteRegister(2, (int) fd);
		}
		kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
		kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
		...
		int fd = Sysread(filename, (int)kernel->machine->ReadRegister(5), (int)kernel->machine->ReadRegister(6));
		kernel->machine->WriteRegister(2, (int) fd);
		}
		kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
		kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
