...
bool PhyPagesta[NumPhysPages] = {false};
...
// how big is address space?
    ...
    for(int i = 0,index = 0 ; i < numPages ; i++) {
        ...
        while(PhyPagesta[index] == true){
            index++;
            if(index == NumPhysPages) break; 
        }
        PhyPagesta[index] = true;
...
// Note: this code assumes that virtual address = physical address
    if (noffH.code.size > 0) {
        ...
        executable->ReadAt(&(kernel->machine->mainMemory[pageTable[noffH.code.virtualAddr/PageSize].physicalPage * PageSize + (noffH.code.virtualAddr%PageSize)]),
            noffH.code.size, noffH.code.inFileAddr);
    }
    if (noffH.initData.size > 0) {
        DEBUG(dbgAddr, "Initializing data segment.");
        DEBUG(dbgAddr, noffH.initData.virtualAddr << ", " << noffH.initData.size);
        executable->ReadAt(&(kernel->machine->mainMemory[pageTable[noffH.initData.virtualAddr/PageSize].physicalPage * PageSize + (noffH.code.virtualAddr%PageSize)]),
